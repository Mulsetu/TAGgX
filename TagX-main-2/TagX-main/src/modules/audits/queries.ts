...
import { ASSET_CONDITIONS, type AssetCondition } from "@/modules/assets/types";
import { listLocationOptions } from "@/modules/locations/queries";
import { AUDIT_EXCEPTION_TYPES, AUDIT_ITEM_STATUSES, AUDIT_STATUSES } from "./types";
import type {
  AuditDetail,
  AuditExceptionType,
  AuditItem,
...
function isExceptionType(value: string): value is AuditExceptionType {
  return (AUDIT_EXCEPTION_TYPES as readonly string[]).includes(value);
}
...
function isCondition(value: string | null): AssetCondition | null {
  ...
  return (ASSET_CONDITIONS as readonly string[]).includes(value) ? (value as AssetCondition) : null;
}
...
const ITEM_SELECT = `
  ...
  notes, scanned_at, resolved, resolution_notes
`;

export async function listAudits(page: number, pageSize = PAGE_SIZE): Promise<AuditListResult> {
  const supabase = createClient();
...
export async function findActiveAuditItemForAsset(assetId: string): Promise<AuditTagContext | null> {
  ...
  return { auditId: campaign.id, auditName: campaign.name, item, locations };
}
