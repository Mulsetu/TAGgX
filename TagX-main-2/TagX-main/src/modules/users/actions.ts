...
import { getCompanyBySlug } from "@/modules/companies/queries";
...
export async function inviteCompanyUserAction(
  ...
  const inviteResult = await createCompanyInvite(company.id, parsed.data.roleId, parsed.data.email, invitedBy?.id ?? null);
