...
function ViewerActions({ asset, viewer }: { asset: PublicAsset; viewer: TagPageViewer }) {
  ...
  return (
    <Button asChild>
      <Link href={`/${asset.company.slug}/login?next=${encodeURIComponent(`/assets/${asset.id}`)}`}>
        Sign in to take more actions
      </Link>
    </Button>
  );
}
...
export default async function PublicAssetTagPage({ params }: TagPageProps) {
  ...
  return (
    <main
      className="mx-auto flex min-h-screen w-full max-w-lg flex-col gap-8 px-4 py-10"
