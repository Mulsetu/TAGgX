...
export function parseCategoryFieldValues(
  ...
  for (const field of fields) {
    ...
    if (field.fieldType === "email" && !/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(value)) {
      fieldErrors[formKey] = `${field.label} must be a valid email.`;
      continue;
    }
    if (field.fieldType === "url") {
      try {
        ...
        fieldErrors[formKey] = `${field.label} must be a valid URL.`;
        continue;
      }
    }
    if (field.fieldType === "phone" && value.length > 30) {
      fieldErrors[formKey] = `${field.label} is too long.`;
      continue;
    }
    if (field.fieldType === "textarea" && value.length > 4000) {
      fieldErrors[formKey] = `${field.label} is too long.`;
      continue;
    }
    values[field.key] = value;
