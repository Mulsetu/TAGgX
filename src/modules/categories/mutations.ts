...
export async function createCategory(
  ...
  const { data, error } = await supabase
    ...
    .insert({
      ...
      code_prefix: input.codePrefix ?? null,
      is_active: input.isActive ?? true,
      default_status_id: input.defaultStatusId ?? null,
      default_condition_key: input.defaultConditionKey ?? null,
    })
    .select("id")
    .single<{ id: string }>();
...
export async function replaceRequiredDocuments(
  ...
  const supabase = createClient();
  await supabase.from("category_required_documents").delete().eq("category_id", categoryId);
  if (keys.length === 0) {
    return { error: null };
  }
  const { error } = await supabase.from("category_required_documents").insert(
    keys.map((key) => ({ company_id: companyId, category_id: categoryId, document_type_key: key })),
  );
