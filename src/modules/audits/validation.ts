...
export const createAuditSchema = z.object({
  ...
  requirePhotoOnException: z.boolean().optional().default(false),
  requireRemarkOnException: z.boolean().optional().default(false),
});
...
export const auditItemListQuerySchema = z.object({
  ...
  exceptionType: z.preprocess(emptyToUndefined, z.string().trim().regex(/^[a-z][a-z0-9_]{0,63}$/).optional()),
});
...
export const recordAuditScanSchema = z.object({
  ...
  foundCondition: z.preprocess(emptyToUndefined, z.string().trim().regex(/^[a-z][a-z0-9_]{0,63}$/).optional()),
  notes: z.preprocess(emptyToUndefined, z.string().trim().max(2000).optional()),
  force: z.preprocess((value) => value === "true" || value === true, z.boolean().optional()),
  extraExceptionTypes: z.array(z.string().trim().regex(/^[a-z][a-z0-9_]{0,63}$/)).optional().default([]),
  foundCustodianId: z.preprocess(emptyToUndefined, z.string().uuid().optional()),
});
