...
function buildAssetFields(input: AssetFormInput, customFields: Record<string, CustomFieldValue>) {
  return {
    ...
    current_book_value: input.currentBookValue ?? null,
    residual_value: input.residualValue ?? null,
...
export async function updateAssetStatus(assetId: string, statusId: string): Promise<{ error: string | null }> {
  const supabase = createClient();
  const { error } = await supabase.from("assets").update({ status_id: statusId }).eq("id", assetId);
  return { error: error ? "Could not update asset status." : null };
}
...
export async function restoreAsset(id: string): Promise<{ error: string | null }> {
  const supabase = createClient();
  const { data, error } = await supabase
    .from("assets")
    ...
    .maybeSingle<{ id: string }>();
  if (error || !data) {
    return { error: "Could not restore the asset." };
  }
  return { error: null };
}

export async function setAssetArchived(id: string, archived: boolean): Promise<{ error: string | null }> {
  const supabase = createClient();
  const { data, error } = await supabase
    .from("assets")
    ...
    .maybeSingle<{ id: string }>();
  if (error || !data) {
    return { error: archived ? "Could not archive the asset." : "Could not unarchive the asset." };
  }
  return { error: null };
}

export async function deleteAssetDocument(id: string): Promise<{ error: string | null; filePath?: string; sizeBytes?: number }> {
  const supabase = createClient();
  const { data, error } = await supabase
    ...
    .maybeSingle<{ file_path: string; file_size_bytes: number }>();
  if (error) {
  ...
  return { error: null, filePath: data.file_path, sizeBytes: data.file_size_bytes };
}
 ...
 */
export async function generateAssetCode(companyId: string, codePrefix?: string | null): Promise<string> {
  const supabase = createClient();
  ...
  const format = codePrefix
    ? `${codePrefix}-{SEQ:05d}`
    : (settings?.asset_code_format ?? "AST-{SEQ:05d}");
...
export async function insertQrEvent(params: {
  companyId: string;
  assetId: string;
  eventType: "generated" | "regenerated" | "printed" | "replaced";
  actorId: string | null;
}): Promise<void> {
  const supabase = createClient();
