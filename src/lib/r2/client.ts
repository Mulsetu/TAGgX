...
const PUBLIC_MEDIA_KEY =
  /^companies\/[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}\/(branding|images)\//i;

/** Logos and asset hero images stay fetchable without a session (unguessable keys). */
export function isPublicCompanyMediaKey(key: string): boolean {
  return isCompanyObjectKey(key) && PUBLIC_MEDIA_KEY.test(key);
}
